...
typedef NS_ENUM(NSUInteger, PCIPOpCode) {
    PCIPOpCodeContinueFrame = 0x0,
    PCIPOpCodeTextFrame = 0x1,
    PCIPOpCodeBinaryFrame = 0x2,
    //3-7 are reserved.
    PCIPOpCodeConnectionClose = 0x8,
    PCIPOpCodePing = 0x9,
    PCIPOpCodePong = 0xA,
    //B-F reserved.
};

typedef NS_ENUM(NSUInteger, PCIPCloseCode) {
    PCIPCloseCodeNormal                 = 1000,
    PCIPCloseCodeGoingAway              = 1001,
    PCIPCloseCodeProtocolError          = 1002,
    PCIPCloseCodeProtocolUnhandledType  = 1003,
    // 1004 reserved.
    PCIPCloseCodeNoStatusReceived       = 1005,
    //1006 reserved.
    PCIPCloseCodeEncoding               = 1007,
    PCIPCloseCodePolicyViolated         = 1008,
    PCIPCloseCodeMessageTooBig          = 1009
};

typedef NS_ENUM(NSUInteger, PCIPInternalErrorCode) {
    // 0-999 WebSocket status codes not used
    PCIPOutputStreamWriteError  = 1
};

#define kPCIPInternalHTTPStatusWebSocket 101

//holds the responses in our read stack to properly process messages
@interface PCIPResponse : NSObject

@property(nonatomic, assign)BOOL isFin;
@property(nonatomic, assign)PCIPOpCode code;
...
@interface PCIPSocket ()<NSStreamDelegate>
...
static const uint8_t PCIPFinMask             = 0x80;
static const uint8_t PCIPOpCodeMask          = 0x0F;
static const uint8_t PCIPRSVMask             = 0x70;
static const uint8_t PCIPMaskMask            = 0x80;
static const uint8_t PCIPPayloadLenMask      = 0x7F;
static const size_t  PCIPMaxFrameSize        = 32;

@implementation PCIPSocket
...
/////////////////////////////////////////////////////////////////////////////
- (void)disconnect {
    [self writeError:PCIPCloseCodeNormal];
}
/////////////////////////////////////////////////////////////////////////////
- (void)writeString:(NSString*)string {
    if(string) {
        [self dequeueWrite:[string dataUsingEncoding:NSUTF8StringEncoding]
                  withCode:PCIPOpCodeTextFrame];
    }
}
/////////////////////////////////////////////////////////////////////////////
- (void)writePing:(NSData*)data {
    [self dequeueWrite:data withCode:PCIPOpCodePing];
}
/////////////////////////////////////////////////////////////////////////////
- (void)writeData:(NSData*)data {
    [self dequeueWrite:data withCode:PCIPOpCodeBinaryFrame];
}
/////////////////////////////////////////////////////////////////////////////
...
- (void)createHTTPRequest {
    ...
    CFDictionaryRef cfHeaders = CFHTTPMessageCopyAllHeaderFields(urlRequest);
    NSDictionary *dict =  CFBridgingRelease(cfHeaders);
    NSLog(@"headerst = \"%@\"", dict);
#endif
    NSData *serializedRequest = (__bridge_transfer NSData *)(CFHTTPMessageCopySerializedMessage(urlRequest));
    NSLog(@"Serialized Request == %@",[[NSString alloc] initWithData:serializedRequest encoding:NSUTF8StringEncoding]);
...
- (BOOL)validateResponse:(uint8_t *)buffer length:(NSInteger)bufferLen responseStatusCode:(CFIndex*)responseStatusCode {
    ...
    *responseStatusCode = CFHTTPMessageGetResponseStatusCode(response);
    BOOL status = ((*responseStatusCode) == kPCIPInternalHTTPStatusWebSocket)?(YES):(NO);
    if(status == NO) {
        CFRelease(response);
        return NO;
    }
...
-(void)processRawMessage:(uint8_t*)buffer length:(NSInteger)bufferLen {
    PCIPResponse *response = [self.readStack lastObject];
    ...
    if(response.bytesLeft > 0) {
        ...
        BOOL isFin = (PCIPFinMask & buffer[0]);
        uint8_t receivedOpcode = (PCIPOpCodeMask & buffer[0]);
        BOOL isMasked = (PCIPMaskMask & buffer[1]);
        uint8_t payloadLen = (PCIPPayloadLenMask & buffer[1]);
        NSInteger offset = 2; //how many bytes do we need to skip for the header
        if((isMasked  || (PCIPRSVMask & buffer[0])) && receivedOpcode != PCIPOpCodePong) {
            [self doDisconnect:[self errorWithDetail:@"masked and rsv data is not currently supported" code:PCIPCloseCodeProtocolError]];
            [self writeError:PCIPCloseCodeProtocolError];
            return;
        }
        BOOL isControlFrame = (receivedOpcode == PCIPOpCodeConnectionClose || receivedOpcode == PCIPOpCodePing);
        if(!isControlFrame && (receivedOpcode != PCIPOpCodeBinaryFrame && receivedOpcode != PCIPOpCodeContinueFrame && receivedOpcode != PCIPOpCodeTextFrame && receivedOpcode != PCIPOpCodePong)) {
            [self doDisconnect:[self errorWithDetail:[NSString stringWithFormat:@"unknown opcode: 0x%x",receivedOpcode] code:PCIPCloseCodeProtocolError]];
            [self writeError:PCIPCloseCodeProtocolError];
            return;
        }
        if(isControlFrame && !isFin) {
            [self doDisconnect:[self errorWithDetail:@"control frames can't be fragmented" code:PCIPCloseCodeProtocolError]];
            [self writeError:PCIPCloseCodeProtocolError];
            return;
        }
        if(receivedOpcode == PCIPOpCodeConnectionClose) {
            //the server disconnected us
            uint16_t code = PCIPCloseCodeNormal;
            if(payloadLen == 1) {
                code = PCIPCloseCodeProtocolError;
            }
        ...
        if(receivedOpcode == PCIPOpCodePong) {
        ...
        if(!isFin && receivedOpcode == PCIPOpCodeContinueFrame && !response) {
            [self doDisconnect:[self errorWithDetail:@"continue frame before a binary or text frame" code:PCIPCloseCodeProtocolError]];
            [self writeError:PCIPCloseCodeProtocolError];
            return;
        }
        BOOL isNew = NO;
        if(!response) {
            if(receivedOpcode == PCIPOpCodeContinueFrame) {
                [self doDisconnect:[self errorWithDetail:@"first frame can't be a continue frame" code:PCIPCloseCodeProtocolError]];
                [self writeError:PCIPCloseCodeProtocolError];
                return;
            }
            isNew = YES;
            response = [PCIPResponse new];
            ...
            if(receivedOpcode == PCIPOpCodeContinueFrame) {
                ...
                [self doDisconnect:[self errorWithDetail:@"second and beyond of fragment message must be a continue frame" code:PCIPCloseCodeProtocolError]];
                [self writeError:PCIPCloseCodeProtocolError];
                return;
            }
...
/////////////////////////////////////////////////////////////////////////////
- (BOOL)processResponse:(PCIPResponse*)response {
    if(response.isFin && response.bytesLeft <= 0) {
        NSData *data = response.buffer;
        if(response.code == PCIPOpCodePing) {
            [self dequeueWrite:response.buffer withCode:PCIPOpCodePong];
        } else if(response.code == PCIPOpCodeTextFrame) {
            NSString *str = [[NSString alloc] initWithData:response.buffer encoding:NSUTF8StringEncoding];
            if(!str) {
                [self writeError:PCIPCloseCodeEncoding];
                return NO;
            }
            __weak typeof(self) weakSelf = self;
            dispatch_async(self.queue,^{
        ...
        } else if(response.code == PCIPOpCodeBinaryFrame) {
            __weak typeof(self) weakSelf = self;
            dispatch_async(self.queue,^{
...
/////////////////////////////////////////////////////////////////////////////
-(void)dequeueWrite:(NSData*)data withCode:(PCIPOpCode)code {
    if(!self.isConnected) {
        return;
    }
    if(!self.writeQueue) {
    ...
    [self.writeQueue addOperationWithBlock:^{
        ...
        NSMutableData *frame = [[NSMutableData alloc] initWithLength:(NSInteger)(dataLength + PCIPMaxFrameSize)];
        uint8_t *buffer = (uint8_t*)[frame mutableBytes];
        buffer[0] = PCIPFinMask | code;
        if(dataLength < 126) {
            buffer[1] |= dataLength;
        ...
        while (true) {
            ...
            if(len < 0 || len == NSNotFound) {
                ...
                if(!error) {
                    error = [strongSelf errorWithDetail:@"output stream error during write" code:PCIPOutputStreamWriteError];
                }
...
- (NSError*)errorWithDetail:(NSString*)detail code:(NSInteger)code userInfo:(NSDictionary *)userInfo
{
    ...
    return [[NSError alloc] initWithDomain:@"PCIPSocket" code:code userInfo:details];
}
/////////////////////////////////////////////////////////////////////////////
- (void)writeError:(uint16_t)code {
    uint16_t buffer[1];
    buffer[0] = CFSwapInt16BigToHost(code);
    [self dequeueWrite:[NSData dataWithBytes:buffer length:sizeof(uint16_t)] withCode:PCIPOpCodeConnectionClose];
}
/////////////////////////////////////////////////////////////////////////////
- (void)dealloc {
    if(self.isConnected) {
